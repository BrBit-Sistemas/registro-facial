...
export const getHeaders = (hasHeaders: boolean): AxiosRequestConfig => hasHeaders ? { headers: { 'Authorization': `Bearer ${getToken().toString().replace(/"/g, "")}` } } : { headers: {} };
...
export const request = {
  ...
  post: <T>(path: string, data: any, hasHeaders = true): Promise<AxiosResponse<T>> => 
    axios.post<T>(urlAppender(path), data, getHeaders(hasHeaders)),
  put: <T>(path: string, data: any, hasHeaders = true): Promise<AxiosResponse<T>> => 
    axios.put<T>(urlAppender(path), data, getHeaders(hasHeaders)),
  delete: <T>(path: string, data?: any, hasHeaders = true): Promise<AxiosResponse<T>> => 
